...
class App extends Component {
  ...
  componentDidMount() {
    ...
    if (token) {
      ...
      axios
        .get(`http://165.22.228.25:3001/api/getHashes${publicAddress}`, {
  ...
  addHash = input => {
    ...
    axios
      .post(
        "http://165.22.228.25:3001/api/putHash",
        { newHashItem },
        {
          headers: { Authorization: "Bearer " + token }
        }
  ...
  deleteHash = id => {
    ...
    axios
      .delete(`http://165.22.228.25:3001/api/deleteHash`, {
        headers,
        data
      })
      .then(res => this.componentDidMount());
    ...
    // if account found submit hash to contract and update database
      ...
      contract.addHash(formatOutput, (error, result) => {
        if (!error) {
          ...
          axios
            .post("http://165.22.228.25:3001/api/addTx", updateHashItem, {
              headers: { Authorization: "Bearer " + token }
            })
